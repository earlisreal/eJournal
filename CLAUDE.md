...
- `ui/chart/canvas/` — app-side glue for the Trade Analysis chart: `CanvasChartAdapter` maps `AnalysisState` onto the chart composable, and `AppChartColors` bridges the app theme. The chart itself is **wickplot** (`io.github.earlisreal:wickplot` from Maven Central; github.com/earlisreal/wickplot) — a native Compose-Canvas candlestick library extracted from this app (2026-07-02); the domain `Bar` implements its `Candle` interface (zero-copy). Version pinned in `gradle/libs.versions.toml`; keep wickplot's Kotlin/Compose/kotlinx-datetime versions in sync with this app's when bumping either. Replaced an earlier Lightweight-Charts-in-JCEF bridge (removed to drop ~130 MB/platform of native CEF payload).
